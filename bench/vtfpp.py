...
def create_plot_compute_mips(exe: str, data: pd.DataFrame) -> None:
    df = data[data['name'].apply(lambda name: 'compute_mips' in name)]
    ...
    df_merged.plot(kind='bar', figsize=(10, 6))
    plt.title('VTFPP - Compute Mips (2048x2048 RGBA8888, Mitchell Filter)')
    plt.xlabel('Type')
    plt.ylabel('Time (ms)')
    plt.xticks(rotation=45, ha='right')
    plt.legend(['vtfpp', 'vtflib'])
    ...
    plt.savefig(str(pathlib.Path(exe).parent / (get_lib_name(__file__) + '_bench_compute_mips_2048x2048_rgba8888_mitchell.png')))
...
def main() -> None:
    ...
    create_plot_compute_mips(exe, data)
