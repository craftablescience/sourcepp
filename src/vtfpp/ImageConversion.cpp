...
namespace vtfpp::detail::DistanceMapping {
namespace {
...
bool operator!(ImageConversion::DistanceFlags flags) {
	return flags == ImageConversion::DistanceFlags::NONE;
};
...
[[nodiscard]] std::vector<std::byte> ImageConversion::alphaToDistance(std::span<const std::byte> imageData, ImageFormat inFormat, ImageFormat outFormat, uint16_t width, uint16_t height, uint16_t reduceX, uint16_t reduceY, bool srgb, float distanceSpread, float alphaThreshold, ImageConversion::DistanceFlags flags, ImageConversion::DistanceDither dither, ImageConversion::ResizeFilter filter, ImageConversion::ResizeEdge edge, bool *valveQuirks) {
	using namespace vtfpp::detail::DistanceMapping;
	using namespace ImageFormatDetails;
	...
	SampleParam param(reinterpret_cast<const float *>(srcImg.data()), width, height, reduceX, reduceY, srcPxLen, srcAlphaOffs, distanceSpread, !!(flags & DistanceFlags::SAMPLECENTERED), alphaThreshold, edge);
	...
	bool doGradientDither = dither == DistanceDither::GRADIENT_TANGENT && !decimal(outFormat);
	paintMap(
		&param,
		reinterpret_cast<float *>(dstImg.data()),
		dstPxLen,
		dstAlphaOffs,
		!!(flags & DistanceFlags::DISTANCEAA),
		!!(flags & DistanceFlags::EUCLIDEAN),
