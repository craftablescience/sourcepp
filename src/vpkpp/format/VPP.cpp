...
std::unique_ptr<PackFile> VPP::open(const std::string& path, const EntryCallback& callback) {
	...
	if (version == 1) {
		...
		// Get package flags
		vpp->packageFlags = reader.read<uint32_t>();

		// Remove compressed flag if we're also condensed
		if (vpp->packageFlags & FLAG_CONDENSED) {
			vpp->packageFlags &= ~FLAG_COMPRESSED;
		}

		// ??
		reader.skip_in(sizeof(uint32_t));
		...
		vpp->entryBaseOffset += entryDirectorySize + sourcepp::math::paddingForAlignment(VPP_ALIGNMENT, entryDirectorySize);
		vpp->entryBaseOffset += entryNamesSize + sourcepp::math::paddingForAlignment(VPP_ALIGNMENT, entryNamesSize);
		...
		for (uint32_t i = 0; i < entryCount; i++) {
			...
			// Not compressed
			if (entry.compressedLength == 0xFFFFFFFF || !(vpp->packageFlags & FLAG_COMPRESSED))
