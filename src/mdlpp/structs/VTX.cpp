...
bool VTX::open(const std::byte* data, std::size_t size, const MDL::MDL& mdl, HasTopologyIndices hasTopologyIndices) {
	const auto openInternal = [this, data, size, checksum = mdl.checksum](bool useTopology) {
		...
		if (stream.read<int32_t>() != checksum) {
			return false;
		}
	...
	switch (hasTopologyIndices) {
		case HasTopologyIndices::AUTOMATIC: {
			const bool useTopology = mdl.version == 49;
			try {
				return openInternal(useTopology);
		...
		case HasTopologyIndices::NO:
			return openInternal(false);
		case HasTopologyIndices::YES:
