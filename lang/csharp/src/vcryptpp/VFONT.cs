...
public static class VFONT
{
	...
	{
		return new sourcepp.Buffer(DLL.vcryptpp_vfont_encrypt(buffer, (ulong) buffer.LongLength, saltLen)).Read();
	}
	...
	{
		return new sourcepp.Buffer(DLL.vcryptpp_vfont_decrypt(buffer, (ulong) buffer.LongLength)).Read();
	}
