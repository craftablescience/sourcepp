...
void register_python(py::module_& m) {
...

	{
		...
		ImageConversion.def("convert_image_data_to_format", [](const py::bytes& imageData, ImageFormat oldFormat, ImageFormat newFormat, uint16_t width, uint16_t height) -> py::bytes {
			...
			const auto d = convertImageDataToFormat({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, oldFormat, newFormat, width, height);
		...
		ImageConversion.def("convert_several_image_data_to_format", [](const py::bytes& imageData, ImageFormat oldFormat, ImageFormat newFormat, uint8_t mipCount, uint16_t frameCount, uint16_t faceCount, uint16_t width, uint16_t height, uint16_t sliceCount) -> py::bytes {
			const std::string_view imageDataView{imageData};
			const auto d = convertSeveralImageDataToFormat({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, oldFormat, newFormat, mipCount, frameCount, faceCount, width, height, sliceCount);
			return {reinterpret_cast<const char*>(d.data()), d.size()};
		...
		ImageConversion.def("convert_image_data_to_file", [](const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t height, FileFormat fileFormat = FileFormat::DEFAULT) -> py::bytes {
			const std::string_view imageDataView{imageData};
			const auto d = convertImageDataToFile({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, fileFormat);
			return {reinterpret_cast<const char*>(d.data()), d.size()};
		...
		ImageConversion.def("convert_file_to_image_data", [](const py::bytes& fileData) -> std::tuple<py::bytes, ImageFormat, int, int, int> {
			...
			const auto d = convertFileToImageData({reinterpret_cast<const std::byte*>(fileDataView.data()), fileDataView.size()}, format, width, height, frame);
			return {{reinterpret_cast<const char*>(d.data()), d.size()}, format, width, height, frame};
		}, py::arg("file_data"));
		...
		ImageConversion.def("resize_image_data", [](const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t newWidth, uint16_t height, uint16_t newHeight, bool srgb, ResizeFilter filter, ResizeEdge edge = ResizeEdge::CLAMP) -> py::bytes {
			const std::string_view imageDataView{imageData};
			const auto d = resizeImageData({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, newWidth, height, newHeight, srgb, filter, edge);
			return {reinterpret_cast<const char*>(d.data()), d.size()};
		...
		ImageConversion.def("resize_image_data_strict", [](const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t newWidth, ResizeMethod widthResize, uint16_t height, uint16_t newHeight, ResizeMethod heightResize, bool srgb, ResizeFilter filter, ResizeEdge edge = ResizeEdge::CLAMP) -> std::tuple<py::bytes, int, int> {
			...
			const auto d = resizeImageDataStrict({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, newWidth, widthOut, widthResize, height, newHeight, heightOut, heightResize, srgb, filter, edge);
			return {{reinterpret_cast<const char*>(d.data()), d.size()}, widthOut, heightOut};
	...
	py::class_<PPL::Image>(vtfpp, "PPLImage")
			...
			.def_property("data", [](const PPL::Image& self) -> py::bytes {
				return {reinterpret_cast<const char*>(self.data.data()), self.data.size()};
			}, [](PPL::Image& self, const py::bytes& data) {
				const std::string_view dataView{data};
				self.data = {reinterpret_cast<const std::byte*>(dataView.data()), reinterpret_cast<const std::byte*>(dataView.data()) + dataView.size()};
			});
	...
	py::class_<PPL>(vtfpp, "PPL")
			...
			.def(py::init([](const py::bytes& pplData) -> PPL* {
				...
				return new PPL{{reinterpret_cast<const std::byte*>(pplDataView.data()), pplDataView.size()}};
			}), py::arg("ppl_data"))
			...
			.def("set_image", [](PPL& self, const py::bytes& imageData, ImageFormat format, uint32_t width, uint32_t height, uint32_t lod = 0) {
				const std::string_view imageDataView{imageData};
				self.setImage({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, lod);
			...
			.def("set_image_resized", [](PPL& self, const py::bytes& imageData, ImageFormat format, uint32_t width, uint32_t height, uint32_t resizedWidth, uint32_t resizedHeight, uint32_t lod = 0, ImageConversion::ResizeFilter filter = ImageConversion::ResizeFilter::BILINEAR) {
				const std::string_view imageDataView{imageData};
				self.setImage({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, resizedWidth, resizedHeight, lod, filter);
	...
	py::class_<VTF>(vtfpp, "VTF")
			...
			.def(py::init([](const py::bytes& vtfData, bool parseHeaderOnly = false) -> VTF* {
				...
				return new VTF{std::span{reinterpret_cast<const std::byte*>(vtfDataView.data()), vtfDataView.size()}, parseHeaderOnly};
			...
			.def_static("create_and_bake", [](const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t height, const std::string& vtfPath, VTF::CreationOptions options) {
				const std::string_view imageDataView{imageData};
				VTF::create({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, vtfPath, options);
			...
			.def_static("create", [](const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t height, VTF::CreationOptions options) {
				const std::string_view imageDataView{imageData};
				return VTF::create({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, options);
			...
			.def("set_particle_sheet_resource", [](VTF& self, const py::bytes& value) {
				...
				return self.setParticleSheetResource({reinterpret_cast<const std::byte*>(valueView.data()), valueView.size()});
			}, py::arg("value"))
			...
			.def("set_image", [](VTF& self, const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t height, ImageConversion::ResizeFilter filter = ImageConversion::ResizeFilter::BILINEAR, uint8_t mip = 0, uint16_t frame = 0, uint8_t face = 0, uint16_t slice = 0) {
				const std::string_view imageDataView{imageData};
				return self.setImage({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height, filter, mip, frame, face, slice);
			...
			.def("set_thumbnail", [](VTF& self, const py::bytes& imageData, ImageFormat format, uint16_t width, uint16_t height) {
				const std::string_view imageDataView{imageData};
				return self.setThumbnail({reinterpret_cast<const std::byte*>(imageDataView.data()), imageDataView.size()}, format, width, height);
