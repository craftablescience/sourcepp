...
function(add_pretty_parser TARGET)
    ...
    if(OPTIONS_C)
        add_library(sourcepp_${TARGET}c SHARED ${OPTIONS_PRECOMPILED_HEADERS} ${OPTIONS_SOURCES} "$<$<BOOL:${WIN32}>:${CMAKE_CURRENT_SOURCE_DIR}/branding/logo.rc>")
        ...
        if(NOT "${TARGET}" STREQUAL "${PROJECT_NAME}")
            target_link_libraries(sourcepp_${TARGET}c PUBLIC sourcepp::${PROJECT_NAME}c)
        endif()
        target_include_directories(sourcepp_${TARGET}c PUBLIC "${CMAKE_CURRENT_SOURCE_DIR}/lang/c/include")
        string(TOUPPER ${TARGET} TARGET_UPPER)
        target_compile_definitions(sourcepp_${TARGET}c PRIVATE ${TARGET_UPPER}_EXPORTING)
        configure_file("${CMAKE_CURRENT_SOURCE_DIR}/lang/c/include/sourceppc/API.h.in" "${CMAKE_CURRENT_SOURCE_DIR}/lang/c/include/${TARGET}c/API.h")
