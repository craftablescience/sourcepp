...
[[nodiscard]] constexpr double besselI0(double x) {
	...
	if (x <= 8.0) {
		...
		return std::exp(x) * chebyshev(y, besselI0A);
	}
	return std::exp(x) * chebyshev(32.0 / x - 2.0, besselI0B) / std::sqrt(x);
}
